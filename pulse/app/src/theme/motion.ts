...
export const spring = {
  ...
  standard: { damping: 15, stiffness: 120, mass: 1 } satisfies WithSpringConfig,

  /** Snappier, for press states that must feel instantaneous. */
  snappy: { damping: 18, stiffness: 260, mass: 0.7 } satisfies WithSpringConfig,

  /** Softer and slower — big hero numbers, progress rings. */
  gentle: { damping: 20, stiffness: 90, mass: 1.1 } satisfies WithSpringConfig,

  /** A touch of overshoot for celebratory moments (goal met, day completed). */
  bouncy: { damping: 10, stiffness: 180, mass: 0.9 } satisfies WithSpringConfig,

  /** No overshoot at all — anything that must not visually cross its target. */
  precise: { damping: 26, stiffness: 200, mass: 1 } satisfies WithSpringConfig,

  /** Chart bars growing on mount. */
  chart: { damping: 14, stiffness: 110, mass: 1 } satisfies WithSpringConfig,
} as const;

/** Timings are reserved for opacity and colour, where springs make no sense. */
export const timing = {
  fast: { duration: 150, easing: Easing.out(Easing.quad) } satisfies WithTimingConfig,
  ...
  /** Long, unhurried loop used by the background mesh blobs. */
  ambient: { duration: 9000, easing: Easing.inOut(Easing.sin) } satisfies WithTimingConfig,
} as const;
